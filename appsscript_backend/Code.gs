...
function buildSystemPrompt(lang, memory) {
  var en = "You are a warm, highly experienced SENIOR PROFESSOR for an Indian " +
    ...
    "If the student asks for a diagram, flowchart, concept map or mind map, output a valid " +
    "Mermaid diagram inside a ```mermaid fenced code block (use 'graph TD' or 'mindmap' syntax, " +
    "keep node labels short, avoid special characters), followed by a one-line explanation.";
  var hi = "Aap ek warm, bahut experienced SENIOR PROFESSOR ho ek Indian Intermediate " +
    ...
    "Agar student diagram, flowchart, concept map ya mind map maange, to ek valid Mermaid diagram " +
    "```mermaid fenced code block ke andar do ('graph TD' ya 'mindmap' syntax, node labels chhote " +
    "rakho, special characters avoid karo), uske baad ek line ka explanation.";
...
function extractDriveId(s) {
  s = (s || '').trim();
  var m = s.match(/[-\w]{25,}/);   // pull the ID out of a full URL if pasted
  return m ? m[0] : '';
}

function doUpload(body) {
  var folderId = extractDriveId(PROPS.getProperty('DRIVE_FOLDER_ID'));
  if (!folderId) return { ok: false, error: 'Drive Folder ID not set. Add it in Setup.' };
  var b64 = (body.data || '');
  ...
  try {
    ...
    return { ok: false, error: 'Cannot open the Drive folder. (1) Check the Folder ID is correct, ' +
             '(2) re-authorize the script: in the Apps Script editor pick the authorizeNow function and click Run, then Allow. [' + e + ']' };
  }
...
/* Run this ONCE from the editor (select authorizeNow → Run → Allow) to grant
   Drive + Sheets + external-request permissions. Fixes DriveApp errors. */
